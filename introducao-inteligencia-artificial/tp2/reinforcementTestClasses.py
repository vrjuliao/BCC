...
class ValueIterationTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for n in self.numsIterationsForDisplay:
            ...
            testPass, stdOutString, fileOutString = self.executeNIterations(grades, moduleDict, solutionDict, n,
                                                                            checkPolicy)
    ...
    def runAgent(self, moduleDict, numIterations):
        agent = moduleDict['valueIterationAgents'].ValueIterationAgent(self.grid, discount=self.discount,
...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def executeNExperiences(self, grades, moduleDict, solutionDict, n):
        ...
        if lastExperience is not None:
            fileOutString += "Agent observed the transition (startState = %s, action = %s, endState = %s, " \
                             "reward = %f)\n\n" % lastExperience
...
class Question6Test(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        studentSolution = moduleDict['analysis'].passo7()
